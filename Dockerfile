# Runpod ComfyUI Wan2.2 + Jupyter — V4.1.4c (AUTO-REF + entrypoint + deps patched)
FROM nvidia/cuda:12.8.0-cudnn-devel-ubuntu22.04

ENV DEBIAN_FRONTEND=noninteractive         PYTHONUNBUFFERED=1         PIP_NO_CACHE_DIR=1         TZ=UTC

# Base tools
RUN apt-get update && apt-get install -y --no-install-recommends         python3 python3-venv python3-pip git git-lfs curl ca-certificates ffmpeg         tini aria2 jq rsync net-tools iproute2 wget unzip &&         git lfs install &&         rm -rf /var/lib/apt/lists/*
...
ARG COMFY_REF=auto   # values: auto | branch | tag | 40-char SHA
...
# --- OS deps for wheels / builds (OpenCV, minimal build toolchain) ---
RUN apt-get update && apt-get install -y --no-install-recommends         build-essential python3-dev pkg-config cmake         libgl1 libglib2.0-0 &&         rm -rf /var/lib/apt/lists/*

# Keep pip toolchain fresh
RUN /venv/bin/pip install --no-cache-dir -U pip setuptools wheel

# (Optional) If you need PyTorch cu128 wheels preinstalled, uncomment:
# RUN /venv/bin/pip install --no-cache-dir --index-url https://download.pytorch.org/whl/cu128     #     torch torchvision torchaudio

# Install Python deps in stable pinned batches (no 'uv')
RUN /venv/bin/pip install --no-cache-dir         diffusers==0.34.0 accelerate==1.10.0 transformers==4.44.2         huggingface-hub==0.24.6 safetensors==0.4.5         einops==0.8.0 timm==1.0.9 peft==0.17.0         ftfy==6.3.1 protobuf==6.31.1 pyloudnorm==0.1.1         sentencepiece==0.2.0

RUN /venv/bin/pip install --no-cache-dir         opencv-python==4.10.0.84 imageio-ffmpeg==0.4.9         aiohttp==3.9.5         gguf==0.17.1

# Create dirs
RUN mkdir -p /workspace /manifests /scripts /opt/ComfyUI/user/default/workflows         /usr/local/bin /workspace/models

# Copy scripts & manifests
...
# Default ENV
ENV ENABLE_JUPYTER=true         JUPYTER_PORT=8888         JUPYTER_DIR=/workspace         JUPYTER_TOKEN=         COMFY_AUTOSTART=true         COMFY_PORT=8188         COMFY_ARGS="--listen 0.0.0.0 --port 8188 --use-sage-attention"         DATA_DIR=/workspace         COMFY_DIR=/opt/ComfyUI         MODELS_DIR=/workspace/models         COMFY_WORKFLOWS_SRC=/workspace/workflows         COMFY_WORKFLOWS_MODE=symlink         CUSTOM_NODES_MANIFEST=/manifests/nodes_manifest.txt         MODELS_MANIFEST=/manifests/models_manifest.txt         WORKFLOWS_MANIFEST=/manifests/workflows_manifest.txt         NODES_INSTALL_MODE=sync         HF_TOKEN=         COMFY_UPDATE_AT_START=false         COMFY_REF_RUNTIME=
